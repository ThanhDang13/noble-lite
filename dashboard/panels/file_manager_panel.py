"""
File Manager panel - Interface for userspace/file-manager/
"""
...
logger = get_logger("file_manager_panel")


class FileManagerPanel(ttk.Frame):
    """Panel for file manager operations."""

    def __init__(self, parent):
        super().__init__(parent)
        self.binary_path = Path(__file__).parent.parent.parent / "userspace/file-manager/file-manager"
        self.runner = None
    ...
    def setup_ui(self):
        ...
        # Title
        title = ttk.Label(self, text="File Manager", font=("TkDefaultFont", 14, "bold"))
        title.pack(pady=10)
        ...
        # Input frame
        input_frame = ttk.LabelFrame(self, text="Target Path", padding=10)
        ...
        ttk.Button(path_frame, text="Browse File", command=self.browse_file).pack(side=tk.LEFT, padx=2)
        ttk.Button(path_frame, text="Browse Dir", command=self.browse_dir).pack(side=tk.LEFT, padx=2)

        # Control buttons
        btn_frame = ttk.LabelFrame(self, text="Operations", padding=10)
        btn_frame.pack(fill=tk.X, padx=10, pady=5)

        ttk.Button(btn_frame, text="Analyze File", command=lambda: self.run_command(['--analyze'])).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Scan Directory", command=lambda: self.run_command(['--scan'])).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Watch (Live)", command=self.watch_directory).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Stop Watch", command=self.stop_runner, state=tk.DISABLED).pack(side=tk.LEFT, padx=5)
    ...
    def run_command(self, args):
        """Run file-manager with given arguments."""
        ...
        full_args = args + [path]
        self.append_output(f"$ {self.binary_path} {' '.join(full_args)}\n", "system")
        logger.info(f"Running: {self.binary_path} {' '.join(full_args)}")

        self.runner = ProcessRunner(self.on_output)

        if self.runner.start([str(self.binary_path)] + full_args):
            self.process_runner_queue()
        else:
            self.append_output("Failed to start process\n", "stderr")

    def watch_directory(self):
        """Start watching a directory (blocking operation)."""
        path = self.path_entry.get().strip()
        if not path:
        ...
        self.append_output(f"$ {self.binary_path} --watch {path}\n", "system")
        self.append_output("Watch mode started. Press 'Stop Watch' to terminate.\n", "system")
        logger.info(f"Starting watch mode: {path}")

        self.runner = ProcessRunner(self.on_output)

        if self.runner.start([str(self.binary_path), '--watch', path]):
    ...
    def stop_runner(self):
        ...
        if self.runner:
            ...
            logger.info("Stopped file-manager process")
